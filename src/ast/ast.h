...
namespace ast
{
    ...
    public:
        ...
        // Methods for visiting nodes from other namespaces - using void* to avoid circular dependencies
        virtual void visitMoveExpr(void *expr) = 0;
        virtual void visitGoExpr(void *expr) = 0;
        virtual void visitRuntimeChannelSendExpr(void *expr) = 0;
        virtual void visitRuntimeChannelReceiveExpr(void *expr) = 0;
        virtual void visitRuntimeSelectStmt(void *stmt) = 0;
    ...
    public:
        ...
        {
            return std::make_shared<SimpleType>(token);
        }
    };

    // Default token for use when no specific token is available
    static const lexer::Token DEFAULT_TOKEN = lexer::Token(lexer::TokenType::IDENTIFIER, "", "", 0, 0);
    ...
    public:
        ...
        TypePtr clone() const override
        {
            std::vector<TypePtr> clonedTypes;
            for (const auto &type : types)
            {
                clonedTypes.push_back(type->clone());
            }
            return std::make_shared<UnionType>(token, clonedTypes);
        }
    ...
    public:
        ...
        TypePtr clone() const override
        {
            return std::make_shared<ClassType>(token, name);
        }
    ...
    /**
     * @brief Go statement for launching goroutines (e.g., go functionCall())
     */
    class GoStmt : public Statement
    {
    public:
        GoStmt(const lexer::Token &token, ExprPtr expression)
            : Statement(token), expression(std::move(expression)) {}

        void accept(Visitor &visitor) override;

        ExprPtr expression; // The expression to execute in a goroutine
    };
    ...
    /**
     * @brief Implementation statement - implements a trait for a type
     */
    ...
    public:
        ...
        ImplStmt(const lexer::Token& token, const std::string& traitName, TypePtr type)
            : Statement(token), traitName(traitName), type(type) {}
